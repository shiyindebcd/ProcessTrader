...
class DoubleMA(multiprocessing.Process):
    def __init__(self, args):
        ...
        if len(args) > 2:
            ...
            self.backtest_end_date = args[3]                        # 回测结束日期            
    ...
    def run(self):  #重写Process类的run函数，你所有的天勤代码都要放在这个函数里面
        ...
        try:        # 循环语句最好放在try里面，如果有异常，直接结束进程即可，以免出现僵尸进程，进程结束后，主进程会在60秒后重启该进程
            while True:
                ...
                if self.api.is_changing(self.klines.iloc[-1], "datetime"):  # 产生新k线:重新计算SMA
                    self.short_avg = ma(self.klines["close"], self.SHORT)  # 短周期
                    self.long_avg = ma(self.klines["close"], self.LONG)  # 长周期

                    # 均线下穿，做空
                    if self.long_avg.iloc[-2] < self.short_avg.iloc[-2] and self.long_avg.iloc[-1] > self.short_avg.iloc[-1]:
