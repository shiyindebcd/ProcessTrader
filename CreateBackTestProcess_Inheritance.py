...
class BackTestWindow(QWidget, UI):  # 创建回测窗口类
    def __init__(self, parent):
        ...
        self.parent.ioModal = self.parent.ioModal   # 创建读写csv文件对象
        self.pid = None
    ...
    def get_tq_account_list(self):  # 获取天勤账号列表
        tq_account_list = []
        data = self.parent.ioModal.read_csv_file(path='./data/tq_account.csv')
        if data.empty:
            pass
        else:
    ...
    def get_backtest_parameters(self):  # 获取创建一个策略回测进程所需参数
        df = self.parent.ioModal.read_csv_file(path='./data/tq_account.csv')
        ...
        if self.comboBox_select_tq_account.currentText():
            if self.comboBox_select_strategy.currentText():
                if self.symbol.text():
                    ...
                    self.label_info.setText('请先输入回测的合约名,该项参数不可缺少')
            else:
                self.label_info.setText('没有可用的策略模板,请先添加策略模版')
        else:
            self.label_info.setText('没有可用的天勤账户,请先添加可用的天勤账户')
    ...
    def stop_backtest_process(self):
        if self.pid:
            ...
            self.label_info.setText('pid为:  ' + str(self.pid) + '  的回测进程： ' + tuple[1]['process_name'] + '     已关闭')
        else:
            self.label_info.setText('回测进程  没有运行  ' )
